...
<form action="queryProcessor"  method="GET" id="HomePage">
...
<button type="button" id="micro" style="display: none"><i class="fa fa-microphone"  aria-hidden="true" ></i></button>
...
<script type="text/javascript">
    const searchForm = document.querySelector("#HomePage");
    const searchFormInput = searchForm.querySelector("#myInput");
	...
	{	
        ...
        if(searchForm)
        	document.querySelector("#micro").style.display = "inline-block";
           // searchForm.insertAdjacentHTML("beforeend", '<button type="button" id="micro"><i class="fa fa-microphone" aria-hidden="true" ></i></button>');
		...
		function resultOfSpeechRecognition(event) {
		    ...
		    for (var i = event.resultIndex; i < event.results.length; ++i) {
		    	if (event.results[i].isFinal) {
	               searchFormInput.value += event.results[i][0].transcript;
	            }
	        }
		}   
