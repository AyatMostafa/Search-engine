...
//        String message = "Ayat Mostafa";
//        response.setContentType("text/html");
//       
//        String page = "<!doctype html> <html> <body> <h2>" + message +" </h2> </body></html>";
//        response.getWriter().println(page);
		...
		try {
		  ...
		  try {
		     ...
		     if(!ifImage)
		    	 urlBag = ranker.filter(urlBag);
		     
		     System.out.println(urlBag.size());
		     cacheFormer.MakeCache(urlBag); 
		     if(ifImage)
		    	 response.sendRedirect(request.getContextPath()+"/imageResults.jsp?page=1");
		     else
		    	 response.sendRedirect(request.getContextPath()+"/SearchResults.jsp?page=1");
		  } catch (IOException e) {
		      e.printStackTrace();
		  } catch (SQLException e) {
		        // TODO Auto-generated catch block
